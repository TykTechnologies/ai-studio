...
func (p *RateLimitingUIPlugin) getServiceAPIDemo(ctx context.Context, payload string) (*pb.CallResponse, error) {
	...
	for i, plugin := range pluginsResp.Plugins {
		pluginList[i] = map[string]interface{}{
			"id":              plugin.Id,
			"name":            plugin.Name,
			"plugin_category": plugin.PluginCategory,
			"is_active":       plugin.IsActive,
			"hook_type":       plugin.HookType,
			"hook_types":      plugin.HookTypes,
		}
	}
...
func (p *RateLimitingUIPlugin) OnInitialize(serviceAPI mgmtpb.AIStudioManagementServiceClient, pluginID uint32, config map[string]string) error {
	log.Printf("Rate Limiting UI Plugin SDK initializing with broker-based service access")
	log.Printf("Plugin ID: %d, Config keys: %d", pluginID, len(config))
	...
	// Initialize mock data with defaults
	p.kvStore = map[string]interface{}{
		"global_settings": map[string]interface{}{
			"storage_type":       "redis",
			"redis_url":          "redis://localhost:6379",
	...
	if redisURL, ok := config["redis_url"]; ok {
		settings := p.kvStore["global_settings"].(map[string]interface{})
		settings["redis_url"] = redisURL
		log.Printf("Using config redis_url: %s", redisURL)
	}

	if defaultLimitStr, ok := config["default_limit"]; ok {
		var limit int
		if _, err := fmt.Sscanf(defaultLimitStr, "%d", &limit); err == nil {
			settings := p.kvStore["global_settings"].(map[string]interface{})
			settings["default_limit"] = limit
			log.Printf("Using config default_limit: %d", limit)
		}
	}

	if defaultWindow, ok := config["default_window"]; ok {
		settings := p.kvStore["global_settings"].(map[string]interface{})
		settings["default_window"] = defaultWindow
		log.Printf("Using config default_window: %s", defaultWindow)
	}

	if enableBurstStr, ok := config["enable_burst"]; ok {
		settings := p.kvStore["global_settings"].(map[string]interface{})
		settings["enable_burst"] = (enableBurstStr == "true")
		log.Printf("Using config enable_burst: %s", enableBurstStr)
	}
...
// OnInitialize is called by the SDK when the plugin is initialized
func (p *RateLimitingUIPluginSDK) OnInitialize(serviceAPI mgmtpb.AIStudioManagementServiceClient, pluginID uint32, config map[string]string) error {
