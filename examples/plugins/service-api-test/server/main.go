...
func (p *ServiceAPITestPluginSDK) OnInitialize(serviceAPI mgmtpb.AIStudioManagementServiceClient, pluginID uint32) error {
	log.Printf("Service API Test Plugin SDK initializing")
	ctx := context.Background()
	req := &pb.InitRequest{
		Config: map[string]string{"plugin_id": fmt.Sprintf("%d", pluginID)},
	}
