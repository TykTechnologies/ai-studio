...
*   **Generative UI:** A client tool of kind `present` (`models.ClientToolKindPresent`), seeded on startup as "Generative UI" by `models.GetOrCreateDefaultClientTools` (called from `ensureDefaults` in `main.go`, alongside the default LLMs) and placed in the Default tool catalogue, lets the model compose cards, facts, tables, charts, alerts, lists and forms from the `@assistant-ui/react-generative-ui` vocabulary. Its definition is stored base64-encoded like every other tool spec; installations first seeded with a raw JSON definition are re-encoded on the next start (`encodeRawClientToolSpecs`).
*   **Client tool authoring:** the Tools page edits approval and form tools with a field builder (`admin/components/tools/client/`: `schemaFields.js` converts fields to and from JSON Schema, `clientToolPresets.js` ships the examples, `ClientToolEditor.js` composes builder, examples, JSON fallback and `ClientToolPreview.js`, which renders the real chat cards with sample arguments). The tool's schema is embedded in the backend (`models/generative_ui_present_schema.json`, regenerated with `ui/admin-frontend/scripts/gen-present-schema.mjs`); the chat renders the tree (`chat-v2/parts/GenerativeUiCard.js`), resolves the call with `{}` once the arguments have arrived, and turns `$action` payloads from interactive elements into the next user message.
...
*   **ChatSession (`chat_session/`)**
    ...
    *   `client_tools.go`: `DecodeToolSpec` is the one place a tool's stored spec is decoded before it joins a session (default tools, dependencies and tools picked in the chat window); a client definition is accepted base64-encoded or as raw JSON. A tool whose spec cannot be decoded never joins a session: the chat-window add and a tool dependency fail with the error, a default tool is left out (logged) so the chat room still starts. The tool / datasource mutation endpoints reply with the session's `client_tools`, so `ChatViewV2` registers the renderer and human-tool name of a client tool added mid-chat without a new session. Client tools are offered to the model as functions (`prepareTools`), parked when called (`handleToolCalls`), persisted as the tool-call row; `resumeWithToolResults` persists the answers and re-calls the model (`callModelAfterTools`, shared with the REST tool loop); unanswered calls are closed with an error result when the user moves on.
