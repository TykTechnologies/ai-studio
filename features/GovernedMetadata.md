...
| Field definition | `models.MetadataFieldDef` (JSON inside the schema) | `key`, `label`, `type`, `required`, `severity` (`error`/`warning`), `vocabulary_slug`, `pattern`, `min`/`max`, `max_length`, `warn_if_past`, `portal_visible`, `gateway_visible`, `order`. |
