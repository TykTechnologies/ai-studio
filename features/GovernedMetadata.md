...
| Object metadata | `models.ObjectMetadata` | One row per object (`object_type`, `object_id`) holding the `values` map, the last validation status/result, and who wrote it. The pair is a unique index, so `DeleteObjectMetadata` removes the row outright (`Unscoped`) rather than soft-deleting it; a tombstone would block every later write for that object. The create path also purges any soft-deleted leftover from databases written before this rule. History lives in the audit table, not in soft-deleted rows. |
...
| Surface | Shape |
|---|---|
| Admin REST (`/api/v1/llms`, `/tools`, `/datasources`) | `data.governed_metadata` (raw values map) and `data.governed_metadata_status` beside `attributes`; input accepts `attributes.governed_metadata` (absent = untouched, `{}` = clear). The metadata row is written after the object, so a metadata write failure on an otherwise successful save is reported as `meta.governed_metadata_error {code: metadata_write_failed \| hook_rejected, detail}` on the 2xx response. The LLM, Tool and Datasource forms read it (`governedMetadataSaveWarning`) and show a warning snackbar instead of the plain success message. |
