...
*   **Unified Router (single endpoint):**
    ...
    *   Implemented in `proxy/unified_router.go` (`NewUnifiedRouterHandler`, `ParseUnifiedModel`). The handler strips the route prefix from `model` (the per-route shims expect bare model names), rewrites the path to `/ai/{routeId}/v1/...`, and dispatches to the existing `/ai/` chain in-process — auth, app→LLM access, `AllowedModels`, and analytics run exactly as for a direct `/ai/` call. Wired in `proxy.createHandler` *before* the credential middleware; the microgateway mounts the prefix in `microgateway/internal/api/router.go` like `/ai/`.
    *   **The base path is configurable, and the whole ingress optional**, because the proxy library is embedded in host gateways (e.g. Tyk's API Gateway) that may already own `/v1` or mount the proxy under a prefix of their own:
        *   `proxy.Config.UnifiedRouterBasePath` / `aigateway.Config.UnifiedRouterBasePath` — move the ingress (default `/v1`, normalized by `proxy.NormalizeUnifiedRouterBasePath`: leading slash added, trailing slash and dot/empty segments cleaned, `/` refused in favour of the default so the ingress can never swallow the whole gateway). Studio: `UNIFIED_ROUTER_PATH`; microgateway: `GATEWAY_UNIFIED_ROUTER_PATH`.
        *   `proxy.Config.DisableUnifiedRouter` / `aigateway.Config.DisableUnifiedRouter` — remove the ingress and the `{base}/models` route entirely; per-route endpoints (`/ai/`, `/llm/`, `/anthropic/`) are unaffected. Studio: `UNIFIED_ROUTER_DISABLED=true`; microgateway: `GATEWAY_UNIFIED_ROUTER_DISABLED=true`.
        *   Hosts that mount the handler themselves read the effective prefix from `Proxy.UnifiedRouterBasePath()` / `aigateway.Gateway.UnifiedRouterBasePath()` (`""` when disabled) instead of hardcoding `/v1` — this is what the microgateway's route mount uses.
        *   The **shim** base path (`/ai/{routeId}/v1/...`) is internal and does **not** move with the ingress: a request to `{base}/chat/completions` is always rewritten to `/ai/{routeId}/v1/chat/completions`.
    *   Errors: authentication runs before route resolution (anonymous callers always get 401 and cannot enumerate route slugs); a `model` without a route prefix, or with unsafe characters in the prefix → 400 explaining the `<vendor>/<model>` format; an unknown prefix (authenticated) → 404 "vendor not found or not supported by your access rights" from the `/ai/` shim; model-level access rejections are left to the downstream shim (403); bodies over 10 MB → 413.
    *   `GET /v1/models` (i.e. `{base}/models`): OpenAI-format model discovery, registered behind the credential middleware and **scoped to the authenticated app's LLM associations** (`proxy.handleUnifiedListModels`). Ids are `"{routeId}/{model}"`, built from each accessible route's `AllowedModels` + `DefaultModel` (deduped, sorted); routes with neither are omitted, and inaccessible routes are never listed. Non-completion requests under `/v1/` pass through the unified router untouched so this route (and future `/v1/` surface) is served by the authenticated chain.
    *   Distinct from the Enterprise Model Router, which registers per-router `/router/{routerSlug}/v1/...` URLs and does pool-based selection; the unified router is a single configured path with direct slug matching.
