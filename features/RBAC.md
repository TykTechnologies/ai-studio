...
- `AppResponse` reports `is_active`, and the switch is enforced on the request path by both planes: the microgateway refuses an inactive app's credentials, and the embedded gateway's `CheckAPICredential` and bearer branches answer `403 app is inactive` (`proxy/credential_validator.go`), so "deactivate" takes effect on Studio traffic and not only at the edge.
- `ToolInput.active` and `AppInput.is_active` are new optional (`*bool`) attributes; until this change neither flag had an admin write path. `AgentConfigRequest.is_active` and `ModelRouterInput.active` became optional too (omitted = unchanged on update; active / inactive respectively on create), so a `PUT`/`PATCH` that leaves the switch out no longer deactivates the object. LLMs, datasources, model routers and plugins default to inactive on insert (pinned by `models.TestPublishableCreateDefaultsToInactive`); tools, apps and agents default to active, so their create handlers deactivate explicitly for callers without `publish`.
- `PATCH /plugins/:id` by a caller admitted through the per-plugin write permission alone is limited to an allowlist of body keys (`config`, `name`, `description`); any other key is refused with `plugins:write`.
...
**Upgrade note.** System roles are recomputed from the catalogue on every boot, so Editor gains `publish` automatically. Custom roles are deliberately left untouched: a custom role holding `llms:write` can no longer set a provider active until an administrator grants it `llms:publish` (same for tools, datasources, agents, model routers and plugin enable/disable). Per-plugin access is unaffected: `plugins:execute` keeps granting every plugin permission.
