...
Status: design 2026-09-16; implemented M1–M7 (see §18). Live verification against a Tyk stack (§16) is still to be run. Reviewed once for codebase and Tyk API consistency. Enterprise Edition only. Target: AI Studio ≥ 2.3, Tyk Dashboard/Gateway ≥ 5.13 (remote MCP proxies), ≥ 5.15 for REST-to-MCP.
...
| M7 Docs + polish | done | `docs/site/docs/tyk-mcp-integration.md` (modes with Dashboard permission recipes, MDCB and tags, publishing, bundles and keys, drift, registration, submissions and handoff, access report, permissions, configuration, dev-stack recipe, limitations), sidebar entry, `features/README.md` entry, `CHANGELOG-v2.3.0.md` draft. Not done: the live end-to-end verification of §16 against the local Tyk stack, and the deferred `per_api`/MCP ACL field check. |
