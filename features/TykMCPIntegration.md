...
Status: design 2026-09-16; implemented M1–M7 (see §18) and verified live against Tyk Dashboard 5.14 + Gateway EE on 2026-09-16 (§19). Reviewed once for codebase and Tyk API consistency. Enterprise Edition only. Target: AI Studio ≥ 2.3, Tyk Dashboard/Gateway ≥ 5.13 (remote MCP proxies), ≥ 5.15 for REST-to-MCP.
...
- `per_api` cannot be combined with any other pin, and an App whose bundles mix `per_api` with anything else is refused at mint with the servers named. **Verified 2026-09-16 on Dashboard 5.14 + Gateway EE:** the Dashboard refuses a policy that mixes `per_api` with `acl`, `rate_limit` or `quota` (`400 'per_api' can be set to true only if 'quota', 'rate_limit' and 'acl' partitions are set to false`) and the Gateway refuses a key that carries a `per_api` policy next to a partitioned one (`403 cannot apply multiple policies when some have per_api set and some are partitioned`); Studio's messages quote these.
...
- **Access policy**: name, this server's `tyk_api_id` in `access_rights` with `versions:["Default"]`, `partitions.acl=true`, `active=true`, `tags:["ai-studio","studio-managed"]`, `meta_data.studio_connection_id`. Per-primitive allow/block lists are **not offered**: verified 2026-09-16 on Dashboard 5.14, `mcp_primitives` and `json_rpc_methods` inside an access right are rejected (`403 Request body malformed`) and `mcp_access_rights` (like any unknown field) is accepted and silently dropped, so nothing persists. The creator writes plain `access_rights` and links to the Dashboard for primitive-level rules; re-check on 5.15+ before offering them.
...
| `GET /tools/import/tyk/connections`, `GET …/connections/:id/apis?q=`, `GET …/connections/:id/apis/:api_id` | `tools:write` — the Tools import wizard (`features/Tools.md`) reads a Dashboard through a connection without needing the Sensitive `tyk-connections` permission; slim connection projection (no token hint), `ListSourceAPIs`, and `GetSourceAPIDocument` (OAS with `upstream.authentication` and credential-looking header transforms masked). Added 2026-09-17 when the old `providers/` import driver was removed. |
...
| M6 Community registration | done | submission `resource_type=mcp_server` (payload schema in `services/tykmcp/submission.go`, `upstream_auth_token` encrypted at rest and `[redacted]` in every response), validation against the connection at create/update without a Dashboard call, reviewer test = Dashboard dry run on full-mode connections (skipped, never the submitter's upstream, elsewhere), two-phase approve (`RegisterFromSubmission` first, durable via `submissions.external_resource_id`/`resource_id`, then the approval transaction; a retry adopts a proxy already tied to the submission, imported on the same listen path, or found on the Dashboard by listen path), handoff on catalogue/broker connections (`pending_platform` row, `registration_handoff` event without secrets, admin notification, `GET /mcp-servers/:id/handoff` masked by default and with the credential only for `execute` holders, audited), `POST /mcp-servers/:id/link` merging the pending row onto the imported proxy, portal form section and submission detail, reviewer page with deployment-target override and publish, server page handoff section with candidates. Not automated: delivery of `registration_handoff` through the webhooks E2E harness (the topic is published on the same bus as every other `system.mcp_server.*` event). |
| M7 Docs + polish | done | `docs/site/docs/tyk-mcp-integration.md` (modes with Dashboard permission recipes, MDCB and tags, publishing, bundles and keys, drift, registration, submissions and handoff, access report, permissions, configuration, dev-stack recipe, limitations), sidebar entry, `features/README.md` entry, `CHANGELOG-v2.3.0.md` draft. The `per_api` and MCP ACL field checks were run live on 2026-09-16 (see §8.2 and §8.3); `registration_handoff` delivery through webhooks is covered by `api/tykmcp_webhook_enterprise_test.go`, which also fixed the event envelope: every `system.mcp_server.*` / `system.mcp_credential.*` / `system.tyk_connection.*` event now carries the standard `{object_type, action, object_id, user_id, timestamp, object}` payload. Live §16 run: see §19. |

| Tools import on connections | done (2026-09-17) | The Tools list's "Import OpenAPI → Tyk Dashboard" path was rebuilt on `tyk_connections` and the enterprise Dashboard client (`GetSourceAPIDocument`, `/tools/import/tyk/*` under `tools:write`, inline quick-connect in the wizard). The pre-existing core `providers/` package (a second, process-global Dashboard driver) and `/api/v1/providers/*` were deleted; CE keeps direct import only. |
...
- `Submission.BeforeSave` encrypts the payload map in place, so a struct saved twice would encrypt the ciphertext again; the MCP approval reloads the submission between its two writes, and the enterprise tests copy the payload before seeding.
- GORM `default:true` on `tyk_connections.accept_handoffs` turns an explicit false into true on insert; `CreateConnection` writes the false back after the insert.
- Linking keeps the Dashboard-derived fields of the imported row and moves the Studio-owned ones (owner, description, tags, privacy score, teams, submission) from the pending row; a differing name becomes an override.
- **Dashboard 5.14 ignores `dryRun` on `POST /api/mcps` and `POST /api/apis/oas`** (verified live 2026-09-16: every spelling created a real API, and a persisted "dry run" on the same listen path made the Gateway rename both proxies with a "Listen path collision"). Studio therefore never sends dry runs: previews are rendered and checked locally (`RegisterPreview.dashboard_validated=false`) and the Dashboard validates on create or push. The capability probe posts an empty body instead: a permitted user gets `400 the payload should contain x-tyk-api-gateway` with nothing persisted (recorded as `mcp_write=ok`, since the permission check runs first), a denied user 401/403.
- A sync re-probes the connection; `applyProbe` merges instead of replacing, so an `unverified` probe result never erases a state a real write proved (found live: `policies_write` fell back to unverified after the first policy was created).
- `mcp_servers` unique indexes (`slug`, `connection_id+tyk_api_id`) are partial over live rows (`deleted_at IS NULL`): soft-deleted servers must not block re-registering or re-importing the same proxy. Found live: the second registration of a name failed with `duplicate key value violates unique constraint "idx_mcp_servers_slug"` after the first had been deleted, leaving an orphan proxy on the Dashboard.
