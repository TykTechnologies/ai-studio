...
*   **Authorization (resource ACL):** After *any* authentication branch succeeds (API key, Bearer app secret, custom-auth plugin, or a request a microgateway auth plugin already authenticated), the target the path names is checked against the app by `targetAllowed` in `proxy/credential_validator.go`, the one implementation of the rule on both the Studio embedded gateway and the edge:
    *   `/llm/{rest|stream|call}/{slug}`: the app holds the LLM, or inherits it as a failover rung of an LLM it holds (`failoverGrantsAccess`, trusted only with the per-process loopback token).
    *   `/ai/{slug}`, `/anthropic/{slug}` and the unified `/v1` (which rewrites to `/ai/{slug}` before auth): the app holds the LLM. Both hops of the `/ai/` bridge are checked; Bedrock is served from the outer hop, which is its only check.
    *   `/datasource/{slug}`: the app holds the datasource. Tools keep their own ACL (`authorizeToolAccess`).
    *   Unknown and ungranted both answer 403 with the same wording ("... not found or not supported by your access rights"), so slugs cannot be enumerated; bad or inactive credentials stay 401 (the API-key branch keeps its historical 401 for an ungranted target). Until this check existed only the API-key branch applied it, so any active app's Bearer secret could call any active LLM by naming it.
...
*   **Filtering:** LLM-specific request filters (`llm.Filters` with `response_filter=false`) are executed by `runRequestFilters` *before* the request is forwarded. Filters are Tengo scripts stored in the `filters` table. Every entry point runs them:
    *   `/llm/rest/` and `/llm/stream/` (native pass-through) via `screenProxyRequestByVendor`, using the LLM vendor's own message extractor.
    *   `/ai/{route}/v1/chat/completions` and the unified router for non-Bedrock vendors on the internal `/llm/call/` loopback hop.
    *   `/ai/{route}/v1/chat/completions` for **Bedrock** LLMs directly in each waterfall rung (`bedrockScreenRequest`, OpenAI-shaped body) and `/anthropic/{route}/v1/messages` in the bridge handlers (`anthropicScreenRequest`, Anthropic-shaped body), because those paths call the AWS SDK and never take the loopback hop. A block is a 400 that does not fail over to the next rung; the rewritten body is what gets translated to Converse.
*   **Loopback response headers:** On the `/ai/` bridge and the unified `/v1`, the SDK driver consumes the inner `/llm/call/` response, so headers set there would otherwise never reach the client. `InternalRoutingTransport` captures the cache headers (`X-Cache` and `X-Cache-*`, which cache plugins set on hits and misses) from the served rung's 2xx loopback response (`loopbackRelay` in `proxy/internal_transport.go`), and the translator writes them on the outer response: before `WriteHeader` when buffered, and with the first frame when streaming. Vendor and transport headers are deliberately not relayed, because they describe the loopback response, not the translated one. A failed rung's headers never leak.
*   **Post-filter logging:** the request body stored in `proxy_logs` is the body *as the filters left it*. The handlers re-read `r.Body` after `screenProxyRequestByVendor` returns, so a redacting filter redacts for the vendor and for the database alike. A *blocked* request is logged with the body that was blocked (status 400, `policy_violation` response), since that is the audit record of what was refused.
