...
*   **Model Usage Visibility:** Answer "who is using model X?" from the provider details page. The "Models in use" table (`GET /analytics/total-cost-per-vendor-and-model?llm_id=`, `analytics.GetTotalCostPerVendorAndModel`) lists every model served through the provider with request count, distinct app count and last-used time, sortable by any column. Clicking a model opens `LLMModelDetails` (`/admin/llms/:id/models?model=`), which charts that model's usage (`GET /analytics/usage` with `model_name`) and lists the apps that called it with owner email and first/last use (`GET /analytics/apps-for-model`, `analytics.GetAppsForModel`). Everything is read from `llm_chat_records`, which both the embedded gateway and the edge analytics pulse populate, so no schema or data-plane change was needed. The view is scoped to the LLM entry (`llm_id`) rather than vendor-wide. Aggregated timestamps (`MAX(time_stamp)`) are scanned through `analytics.ScanTime` because SQLite returns them as text.
*   **Body Logging Controls:** `llm.DontLogBodies` blanks both bodies on every `ProxyLog` the proxy writes for that LLM, and is synced to edges in the config snapshot. On the microgateway there is also a gateway-wide switch: `ANALYTICS_STORE_REQUESTS` / `ANALYTICS_STORE_RESPONSES` (default `false`, truncated to `ANALYTICS_MAX_BODY_SIZE`) decide what is stored on the edge's `analytics_events` (`requestBodyToStore` / `responseBodyToStore` in `analytics_handler.go`). The pulse is given the row as it will be stored (built in memory by `RecordExchange` before the batched writer inserts it), so `include_request_response_data` can only send bodies both switches allow.
...
*   **Failover Waterfalls (`proxy/failover.go`, `services/llm_failover_validate.go`):**
    ...
    *   **Observability:** the client gets `X-Tyk-Served-LLM`, `X-Tyk-Served-Model` and `X-Tyk-Failover: true`; the response `model` echoes the model that answered. Every attempt that gets an upstream answer writes its own `ProxyLog` (a rung abandoned on the attempt deadline or on a connection error does not, since its inner hop is cancelled before the analyzer runs), fallback rows carrying `FailoverFromLLMID`/`FailoverAttempt`. The proxy-log endpoints serialise the marker as `failover_attempt` and `failover_from_llm_id` (omitted when nil) alongside `llm_id` and `model_name` (`models.ProxyLogAttributes`, built by `NewProxyLogResponse`); the LLM and App detail pages render it in a Failover column (`FailoverCell`). The served headers are written with the documented case (`X-Tyk-Served-LLM`, assigned directly rather than via `Header.Set`, which would canonicalise it to `X-Tyk-Served-Llm`) and, with `X-Tyk-Served-Model` and `X-Tyk-Failover`, are listed in `Access-Control-Expose-Headers` so browser clients can read them. The marker is carried edge→hub in `AnalyticsEvent.failover_from_llm_id/failover_attempt`; on the microgateway it has to survive the plugin hand-off (`interfaces.AnalyticsData.FailoverFromLLMID/FailoverAttempt`, filled from the merged analytics event in `analytics_handler.go`) or the pulse plugin buffers an unmarked event; request counts should filter `failover_attempt = 0`. Metric `aistudio_llm_failover_total{from_llm,to_llm,reason}`; span attributes `aistudio.failover.*`.
