...
func TestPortalCatalog_RespectsVisibility(t *testing.T) {
	...
	assert.Equal(t, map[string]int{"llm": 1, "datasource": 1, "tool": 1, "mcp_server": 0, "model_router": 0, "plugin_resource": 0}, response.Meta.Counts)
...
func TestPortalCatalog_QuerySortAndPage(t *testing.T) {
	...
	t.Run("default is every item newest first with facets", func(t *testing.T) {
		...
		assert.Equal(t, map[string]int{"llm": 3, "datasource": 0, "tool": 1, "mcp_server": 0, "model_router": 0, "plugin_resource": 0}, r.Meta.Counts)
		kinds := []string{}
