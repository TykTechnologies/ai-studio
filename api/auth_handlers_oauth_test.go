...
func (m *MockOAuthClientService) CreateClient(name string, redirectURIs []string, userID uint, scope string) (*models.OAuthClient, string, error) {
	args := m.Called(name, redirectURIs, userID, scope); if args.Get(0) == nil { return nil, args.String(1), args.Error(2) }; return args.Get(0).(*models.OAuthClient), args.String(1), args.Error(2)
}
func (m *MockOAuthClientService) GetClient(clientID string) (*models.OAuthClient, error) {
	args := m.Called(clientID); if args.Get(0) == nil { return nil, args.Error(1) }; return args.Get(0).(*models.OAuthClient), args.Error(1)
}
func (m *MockOAuthClientService) ValidateClientSecret(client *models.OAuthClient, secret string) (bool, error) {
	args := m.Called(client, secret); return args.Bool(0), args.Error(1)
}
func (m *MockOAuthClientService) ValidateRedirectURI(client *models.OAuthClient, redirectURI string) (bool, error) {
	args := m.Called(client, redirectURI); return args.Bool(0), args.Error(1)
}
...
func (m *MockAuthCodeService) CreateAuthCode(argsIn services.CreateAuthCodeArgs) (*models.AuthCode, string, error) {
	args := m.Called(argsIn); if args.Get(0) == nil { return nil, args.String(1), args.Error(2) }; return args.Get(0).(*models.AuthCode), args.String(1), args.Error(2)
}
func (m *MockAuthCodeService) GetValidAuthCodeByCode(codeValue string) (*models.AuthCode, error) {
	args := m.Called(codeValue); if args.Get(0) == nil { return nil, args.Error(1) }; return args.Get(0).(*models.AuthCode), args.Error(1)
}
func (m *MockAuthCodeService) MarkAuthCodeAsUsed(codeValue string) error { return m.Called(codeValue).Error(0) }
...
func (m *MockAccessTokenService) CreateAccessToken(argsIn services.CreateAccessTokenArgs) (*models.AccessToken, string, error) {
	args := m.Called(argsIn); if args.Get(0) == nil { return nil, args.String(1), args.Error(2) }; return args.Get(0).(*models.AccessToken), args.String(1), args.Error(2)
}
func (m *MockAccessTokenService) GetValidAccessTokenByToken(tokenValue string) (*models.AccessToken, error) {
	args := m.Called(tokenValue); if args.Get(0) == nil { return nil, args.Error(1) }; return args.Get(0).(*models.AccessToken), args.Error(1)
}
...
func (m *MockPendingAuthRequestService) StorePendingAuthRequest(argsIn services.StorePendingAuthRequestArgs) (*models.PendingOAuthRequest, error) {
	args := m.Called(argsIn); if args.Get(0) == nil { return nil, args.Error(1) }; return args.Get(0).(*models.PendingOAuthRequest), args.Error(1)
}
func (m *MockPendingAuthRequestService) GetPendingAuthRequest(id string, userID uint) (*models.PendingOAuthRequest, error) {
	args := m.Called(id, userID); if args.Get(0) == nil { return nil, args.Error(1) }; return args.Get(0).(*models.PendingOAuthRequest), args.Error(1)
}
func (m *MockPendingAuthRequestService) DeletePendingAuthRequest(id string) error { return m.Called(id).Error(0) }
...
func (m *AuthServiceMock) Login(c *gin.Context, email, password string) error { return m.Called(c, email, password).Error(0) }
func (m *AuthServiceMock) Logout(c *gin.Context) error { return m.Called(c).Error(0) }
func (m *AuthServiceMock) Register(email, name, password string, withPortal, withChat bool) error { return m.Called(email, name, password, withPortal, withChat).Error(0) }
func (m *AuthServiceMock) ResetPassword(email string) error { return m.Called(email).Error(0) }
func (m *AuthServiceMock) ValidateResetToken(token string) (*models.User, error) { args := m.Called(token); if args.Get(0) == nil { return nil, args.Error(1) }; return args.Get(0).(*models.User), args.Error(1) }
func (m *AuthServiceMock) UpdatePassword(user *models.User, oldPassword, newPassword string) error { return m.Called(user, oldPassword, newPassword).Error(0) }
func (m *AuthServiceMock) VerifyEmail(token string) error { return m.Called(token).Error(0) }
func (m *AuthServiceMock) ResendVerificationEmail(email string) error { return m.Called(email).Error(0) }
func (m *AuthServiceMock) AdminOnly() gin.HandlerFunc { m.MethodCalled("AdminOnly"); return func(c *gin.Context) { c.Next() } }
func (m *AuthServiceMock) SSOOnly() gin.HandlerFunc { m.MethodCalled("SSOOnly"); return func(c *gin.Context) { c.Next() } }
func (m *AuthServiceMock) SSOAuthMiddleware() gin.HandlerFunc { m.MethodCalled("SSOAuthMiddleware"); return func(c *gin.Context) { c.Next() } }
...
func TestHandleOAuthMetadata(t *testing.T) {
	...
	err := json.Unmarshal(w.Body.Bytes(), &metadata); require.NoError(t, err)
...
func TestHandleRegisterOAuthClient_Success(t *testing.T) {
	...
	err := json.Unmarshal(w.Body.Bytes(), &resp); require.NoError(t, err)
...
func TestHandleRegisterOAuthClient_InvalidBody(t *testing.T) {
	...
	err := json.Unmarshal(w.Body.Bytes(), &respBody); require.NoError(t, err)
...
func TestHandleOAuthAuthorize_SuccessRedirectsToConsent(t *testing.T) {
    ...
    appConf := config.Get(); originalSiteURL := appConf.SiteURL
    appConf.SiteURL = "http://dashboard.example.com"; defer func() { appConf.SiteURL = originalSiteURL }()
    ...
    location := w.Header().Get("Location"); require.True(t, strings.HasPrefix(location, "http://dashboard.example.com/oauth/consent?auth_req_id="), "Unexpected redirect URL: "+location)

	pendingReqService := services.NewPendingAuthRequestService(testDB)
	parsedLocation, _ := url.Parse(location); authReqID := parsedLocation.Query().Get("auth_req_id")
	_, err = pendingReqService.GetPendingAuthRequest(authReqID, currentUser.ID); require.NoError(t, err)
}
...
func TestHandleOAuthAuthorize_ValidationErrors(t *testing.T) {
	...
	testCases := []struct {name string; queryString string; expectedErrorSubstring string}{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, http.StatusBadRequest, w.Code)
			var resp gin.H; err := json.Unmarshal(w.Body.Bytes(), &resp); require.NoError(t, err)
...
func TestHandleOAuthToken_ValidCode(t *testing.T) {
	...
	require.NoError(t, dbUser.SetPassword("password123")); require.NoError(t, testDB.Create(&dbUser).Error)

	oauthClientService := services.NewOAuthClientService(testDB)
	client, plainSecret, err := oauthClientService.CreateClient("TokenTestClient", []string{"http://client.example.com/cb"}, dbUser.ID, "mcp"); require.NoError(t, err)

	authCodeService := services.NewAuthCodeService(testDB)
	codeVerifier := "testverifier"; realChallenge := helpers.CalculatePKCEChallengeS256(codeVerifier)
	...
	authCodeInstance, codeValue, err := authCodeService.CreateAuthCode(codeArgs); require.NoError(t, err); require.NotNil(t, authCodeInstance)
...
func TestHandleOAuthToken_ErrorCases(t *testing.T) {
    _, router, _, _, _, _, _ := setupTestAPIWithMocks(t)
    testCases := []struct {name string; formData url.Values; expectedCode int; expectedErrDescSubstring string}{
...
func TestHandleSubmitConsent_Approved(t *testing.T) {
	...
	location := w.Header().Get("Location"); require.Contains(t, location, "http://client.com/cb?"); require.Contains(t, location, "code=")
}
...
func TestHandleSubmitConsent_Denied(t *testing.T) {
	...
	w := performOAuthRequest(router, "POST", "/oauth/submit_consent", input, nil)
	require.Equal(t, http.StatusFound, w.Code)
	location := w.Header().Get("Location"); require.Contains(t, location, "http://client.deny.com/cb?"); require.Contains(t, location, "error=access_denied")
}
