// services/plugin_event_service.go
// Host-side implementation of PluginEventService for AI Studio (control instance).
...
// PluginEventServer implements the PluginEventServiceServer interface for AI Studio.
...
type PluginEventServer struct {
	...
	// nodeID identifies this node (typically "control" for AI Studio)
	nodeID string
	...
	// Key is the subscription ID, value is the pubsub subscription
...
func (s *PluginEventServer) Publish(ctx context.Context, req *pb.PublishRequest) (*pb.PublishResponse, error) {
	...
	log.Debug().
		...
		Msg("Plugin published event to AI Studio bus")
...
func (s *PluginEventServer) Subscribe(req *pb.SubscribeRequest, stream grpc.ServerStreamingServer[pb.EventMessage]) error {
	...
	log.Debug().
		...
		Msg("Plugin subscribed to AI Studio events")
