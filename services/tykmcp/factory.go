...
type Deps struct {
	DB *gorm.DB
	// Bus is the in-process event bus. Nil means events are not published on
	// this node.
	Bus eventbridge.Bus
	...
	Config config.TykMCPConfig
	// NodeID identifies this Studio node in sync leases and logs.
	NodeID string
