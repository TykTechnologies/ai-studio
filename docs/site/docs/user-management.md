...
*   **API Key:** A unique, long-lived token tied to a user. Applications use this key (in an `Authorization: Bearer <key>` header) to authenticate requests on behalf of that user, with that user's full permissions. Only self-registered users receive a key automatically; administrator-created and SSO-provisioned users have none until an administrator issues one. Keys for SSO-provisioned users are off by default (`ALLOW_SSO_USER_API_KEYS`) and, when allowed, only work while the user keeps signing in through the identity provider (`SSO_API_KEY_LIVENESS`, default 30 days as `720h`).
*   **Origin:** Every user records how the account was created: **Self-registered**, **Admin-created** or **SSO**. The Users list shows and filters on it.
...
*   **Status Management:** Disable or enable user accounts. A disabled user is refused everywhere at once: browser sessions, API key, password login, SSO login, OAuth tokens and password reset. Disabling also ends their live session and deactivates the credentials of the apps they own (those are re-activated per app, not by enabling the user).
*   **API Key Management:** Issue, regenerate or revoke a user's API key from the user detail page, and see when it was last used. The Users list shows which accounts hold a key.
...
*   **API Key Authentication:**
    ...
    3.  The application includes the key in the `Authorization` header for requests to Tyk AI Studio APIs:
        ...
        ```
    4.  Tyk AI Studio validates the key and associates the request with its user. The key is refused if the account is disabled, and (for SSO-provisioned users) if the user has not signed in through the identity provider within the liveness window.
    5.  In the Enterprise edition every audited action records whether it was performed through a browser session or an API key (`auth_method`), so key-driven activity can be filtered in the audit trail.
