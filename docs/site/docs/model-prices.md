...
5. **Cost per Cache Write Token** *(Optional)*:
   - The price charged for tokens written to the LLM's prompt cache.
   - Used for models that support prompt caching (e.g., Anthropic Claude with prompt caching).
   - If not set, cache write costs default to the input token price.

6. **Cost per Cache Read Token** *(Optional)*:
   - The price charged for tokens read from the LLM's prompt cache (cache hits).
   - Typically significantly lower than input token costs.
   - If not set, cache read costs default to the input token price.
...
5. **Cost per Cache Write Token** *(Decimal, Optional)*:
   - The price charged per token written to the prompt cache.
   - Leave empty to default to input token pricing.

6. **Cost per Cache Read Token** *(Decimal, Optional)*:
   - The price charged per cached token read (cache hits).
   - Typically much lower than input costs (e.g., 10% of input token price).

7. **Currency** *(Text, Required)*:
