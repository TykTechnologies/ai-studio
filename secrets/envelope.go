...
func NewEnvelopeCipher(wrapper KeyWrapper, keyStore KeyStore) *EnvelopeCipher {
...
func (c *EnvelopeCipher) Encrypt(ctx context.Context, _ []byte, plaintext []byte) ([]byte, error) {
	...
	encoded := fmt.Sprintf("%d/%s", encKey.ID, base64.URLEncoding.EncodeToString(ciphertext))
...
func EncryptEnvelope(ctx context.Context, c *EnvelopeCipher, plaintext string) (string, error) {
	...
	ciphertext, err := c.Encrypt(ctx, nil, []byte(plaintext))
	if err != nil {
		return "", err
	}

	return "$ENC/v2/" + string(ciphertext), nil
}
